...
def do_train(cfg: CfgNode, model: RobertaISTS, train_loader: DataLoader, optimizer: Optimizer, losses: list) -> None:
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            out_vector = model(inputs[0])
            
            loss = losses[0](out_vector, value) # [0] to chyba MSE
            ...
            if i % log_period == log_period - 1:
                logger.info('EPOCH: [%d/%d] BATCHES [%d/%d] loss : %.3f' %
                            (epoch + 1, epochs, i + 1, len(train_loader), loss / log_period))

        logger.info('EPOCH: [%d] FINISHED loss summed: %.3f' %
