...
class RobertaISTS(torch.nn.Module):
    ...
    def __init__(self, num_classes: int, dropout_rate: float, hidden_neurons: int) -> None:
        ...
        self.linear1 = nn.Linear(in_features=self.IN_FEATURES, out_features=hidden_neurons, bias=False)
        self.linear2 = nn.Linear(in_features=hidden_neurons, out_features=num_classes, bias=False)
        self.droput = nn.Dropout(p=dropout_rate, inplace=False)
    ...
    def forward(self, x: Tensor) -> tuple[Tensor, Tensor]:  # not sure whether the first val of the tuple is a Tensor
        ...
        out = self.linear2(self.droput(self.linear1(x)))
        return out
